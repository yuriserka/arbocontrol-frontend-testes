...
const NavBar = function () {
  ...
  this.exibir = async function () {
    ...
    const btn_navbar_lateral = By
      .xpath("(//*[@class='mat-icon notranslate material-icons mat-icon-no-color'])[1]");
    await element(btn_navbar_lateral).click();
  }

  this.expandirRedeSaude = async function () {
    this.exibir()
    const rede_saude_exibida = await this.isRedeSaudeExpandida();
    if (rede_saude_exibida) {
      return;
    }
    await element(this.botoes.rede_de_saude).element(By.xpath(".//div[@class='mat-list-item-content']")).click();
  }

  this.isNavBarExibida = async function () {
    const displayed = await element(By.xpath("/html/body/app-root/app-main-nav/mat-sidenav-container/mat-sidenav/div"))
      .isDisplayed();
    return displayed;
  }
  ...
  this.isRedeSaudeExpandida = async function () {
    ...
    try {
      const class_attr = await element(By.xpath("(//div[contains(@class, 'submenu ng-star-inserted')])[1]")).getAttribute('class');
      if (class_attr.includes("expanded")) {
        return true
      }
    } catch (err) {
    }
