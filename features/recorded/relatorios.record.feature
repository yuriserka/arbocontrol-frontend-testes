...
@Record
Funcionalidade: Gravar requisições http para criação e exclusão de Relatórios

@Blaze
Cenário: Login no BlazeMeter e inicio da gravação
    Dado que eu desejo obter um script de carga para a funcionalidade "relatorios"
...
Esquema do Cenário: Criação de recursos necessários para a edição da lista de trabalho
    Dado que o usuario atual pode editar novas atividades criadas
